...
namespace kinematics
{
    ...
    int &numOfSolns)
  {
    ...
    {
      // std::cout<<"Found ik solutions: "<< num_of_solutions<<std::endl;
      // const IkSolutionBase<IKREAL_TYPE> &sol = solutions.GetSolution(0);
      const IkSolutionBase<IKREAL_TYPE> &sol = solutions.GetSolution(0);
      
      std::vector< double > joints_sol_each(6);
      robot_state::RobotState &current_state = ps.getCurrentStateNonConst();
      for (size_t i = 0; i < num_of_solutions; i++)
      {
        const IkSolutionBase<IKREAL_TYPE> &sol2 = solutions.GetSolution(i);
        sol2.GetSolution(&joints[0], NULL);
        // for (double x : joints)
        //   cout << x << " ";
        // std::cout << std::endl;

        // check collision for joints
        current_state.setVariablePositions(joints);
        c_result.clear();
        ps.checkSelfCollision(c_request, c_result);
        if (!c_result.collision)
          return true;
      }
      return false;
    }
